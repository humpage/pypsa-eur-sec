...
def add_heating_capacities_installed_before_baseyear(n, baseyear, grouping_years, ashp_cop, gshp_cop, time_dep_hp_cop,
                                                     costs, default_lifetime):
    """
    Parameters
    ----------
    n : pypsa.Network
    ...
    for name in names:
        ...
        nodes[name] = pd.Index([n.buses.at[index, "location"] for index in
                                n.buses.index[n.buses.index.str.contains(name) & n.buses.index.str.contains('heat')]])
        ...
        for i, grouping_year in enumerate(grouping_years):
            ...
            n.madd("Link",
                   ...
                   capital_cost=costs.at[name_type + ' resistive heater', 'efficiency'] * costs.at[
                       name_type + ' resistive heater', 'fixed'],
                   p_nom=0.5 * nodal_df[f'{heat_type} resistive heater'][nodes[name]] * ratio / costs.at[
                       name_type + ' resistive heater', 'efficiency'],
                   build_year=int(grouping_year),
                   lifetime=costs.at[costs_name, 'lifetime']
                   )
            ...
            n.madd("Link",
                   ...
                   p_nom=0.5 * nodal_df[f'{heat_type} oil boiler'][nodes[name]] * ratio / costs.at[
                       'decentral oil boiler', 'efficiency'],
                   build_year=int(grouping_year),
                   lifetime=costs.at[name_type + ' gas boiler', 'lifetime']
                   )
            ...
            n.mremove("Link", [index for index in n.links.index.to_list() if
                               str(grouping_year) in index and np.isnan(n.links.p_nom[index])])
